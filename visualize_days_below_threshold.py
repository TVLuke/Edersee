...
Für jedes Jahr ab 1924 wird die Anzahl der Tage in den letzten 10 Jahren angezeigt,
an denen der Wasserstand unter 223,28m fiel (Ende Stützbetrieb für die Oberweser).
"""
import json
import pandas as pd
...
# Konstanten
OBERWESER_THRESHOLD = 223.28  # Wasserstandsschwellenwert für "Ende Stützbetrieb für die Oberweser"
START_YEAR = 1924  # Erstes Jahr zur Analyse
WINDOW_SIZE = 10  # Anzahl der Jahre, die zurückgeblickt werden

# Dateipfade
ACTUAL_DATA_FILE = 'pegelonline-edertalsperre.json'

def load_actual_data(file_path):
    """Lade Wasserstandsdaten aus JSON-Datei."""
...
def load_extracted_data():
    """Lade extrahierte Wasserstandsdaten aus CSV-Dateien."""
...
def combine_data(actual_df, extracted_df):
    """Kombiniere tatsächliche und extrahierte Daten."""
...
def calculate_days_below_threshold(df):
    """
    Berechnet für jedes Jahr ab START_YEAR, wie viele Tage in den
    vorherigen WINDOW_SIZE Jahren Wasserstände unter dem Schwellenwert hatten.
    """
...
def create_days_below_threshold_visualization(days_history, output_file=None):
    """Erstellt eine Visualisierung der Historie der Tage unter dem Schwellenwert."""
...
def main():
    ...
    days_history = calculate_days_below_threshold(combined_df)
    
    # Create visualization
    print("\nErstelle Visualisierung...")
    create_days_below_threshold_visualization(days_history, 'edertalsperre_tage_unter_schwellenwert.png')
